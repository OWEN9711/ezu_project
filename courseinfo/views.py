...
from courseinfo.forms import InstructorForm, SectionForm, CourseForm, RegistrationForm, StudentForm, SemesterForm
from courseinfo.models import Instructor, Section, Course, Registration, Semester, Student
...
class CourseDelete(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    ...
    def get(self, request, pk):
        ...
        if sections.count() > 0:
            return render(request, 'courseinfo/course_refuse_delete.html', {'course':course, 'sections': sections})
        else:
...
class SemesterCreate(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
...
class SemesterDelete(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    ...
    def get(self, request, pk):
        ...
        if sections.count() > 0:
            return render(request, 'courseinfo/semester_refuse_delete.html', {'semester':semester, 'sections':sections})
        else:
...
class StudentDelete(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):
    ...
    def get(self, request, pk):
        ...
        if registrations.count() > 0:
            return render(request, 'courseinfo/student_refuse_delete.html', {'student': student, 'registrations': registrations})
        else:
